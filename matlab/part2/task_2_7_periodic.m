...
rep_x_schr = reshape(repmat(x_schr, 1, rep), 1, rep*N); % Repeat the signal
rep_x_const = reshape(repmat(x_const, 1, rep), 1, rep*N); % Repeat the signal
rep_x_unif = reshape(repmat(x_unif, 1, rep), 1, rep*N); % Repeat the signal
rep_x_rand = reshape(repmat(x_rand, 1, rep), 1, rep*N); % Repeat the signal

% % Plot (time domain)
% figure;
% plot((0:length(rep_x_schr)-1)/fs, rep_x_schr);
